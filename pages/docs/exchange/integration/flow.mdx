...
![High level flow](/exchange/high-level-flow.png)
...
2. The provider's LiveApp starts the Ledger swap process through a call to ExchangeSDK method call: `swap`.

3. After asking the user to start the process, the Ledger Swap Service will call the Provider's backend to generate and sign the `payload`. The backend will also returns a `swapId`, for [status tracking purpose](#check-swap-status-flow).

4. The Provider's backend will generate the payload with the Ledger currency id. This part is important as the Ledger device only knows the Ledger referential currency.
...
6. As soon as the user signs the transaction within his Ledger device, Ledger Live will broadcast the transaction.
...
<Zoom>
    <img
      ...
      src="/exchange/high-level-architecture.png"
    />
</Zoom>
